...
class AppStore extends Component {
  state = {actualAppList: appsList}

  onInputChange = event => {
    const {actualAppList} = this.state

    const searchedList = actualAppList.filter(each =>
      each.appName.toLowerCase().includes(event.target.value.toLowerCase()),
    )
    this.setState({actualAppList: searchedList})
    return searchedList
  }

  updateAppList = id => {
    const filteredAppList = appsList.filter(each => each.category === id)
    this.setState({actualAppList: filteredAppList})
  }
